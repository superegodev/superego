...
it("generates the correct contentSummaryGetter for the tableColumns", async () => {
  ...
  expect(contentSummaryGetter.source).toMatchInlineSnapshot(`
    ...
    export default function getContentSummary(
      ...
      return {
        ...
        "2. Age": String(contact?.age) ?? "",
        "3. Favorite": String(contact?.favorite) ?? "",
        "4. Last Talked At": LocalInstant.fromISO(contact?.lastTalkedAt).toFormat() ?? "",
        "5. Main Phone": contact?.phones?.[0]?.number ?? "",
        "6. Best Friend": contact?.friendGroups?.[0]?.[0] ?? "",
        "7. Second Best Friend": contact?.friendGroups?.[0]?.[1] ?? "",
        "8. \\"Header \\"with\\" quotes\\"": contact?.type ?? "",
        "9. 1Header starting with a number": contact?.type ?? "",
        "10. %Header with odd chars (case 1)": contact?.type ?? "",
        "11. \`Header with odd chars (case 2)": contact?.type ?? "",
        "12. [] syntax (case 1)": contact?.phones?.[0]?.number ?? "",
        "13. [] syntax (case 2)": contact?.name?.first ?? "",
        "14. [] syntax (case 3)": contact?.name?.first ?? "",
        "15. Nested [] syntax (case 1)": contact?.friendGroups?.[0]?.[0] ?? "",
        "16. Nested [] syntax (case 2)": contact?.phones?.[0]?.number ?? "",
        "17. Nested [] syntax (case 3)": contact?.phones?.[0]?.number ?? "",
      };
    }"
  `);
  ...
  expect(contentSummaryResult).toEqual({
    data: {
      ...
      "2. Age": "199",
      "3. Favorite": "true",
      "4. Last Talked At": LocalInstant.fromISO(
        exampleDocumentContent.lastTalkedAt,
      ).toFormat(),
      "5. Main Phone": "+1200000000000",
      "6. Best Friend": "Abel",
      "7. Second Best Friend": "",
      '8. "Header "with" quotes"': "Person",
      "9. 1Header starting with a number": "Person",
      "10. %Header with odd chars (case 1)": "Person",
      "11. `Header with odd chars (case 2)": "Person",
      "12. [] syntax (case 1)": "+1200000000000",
      "13. [] syntax (case 2)": "Bernhard",
      "14. [] syntax (case 3)": "Bernhard",
      "15. Nested [] syntax (case 1)": "Abel",
      "16. Nested [] syntax (case 2)": "+1200000000000",
      "17. Nested [] syntax (case 3)": "+1200000000000",
    },
    error: null,
    success: true,
  });
