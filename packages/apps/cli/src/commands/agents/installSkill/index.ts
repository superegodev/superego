...
import { runCommand, successfulResult } from "../../shared/results.js";
...
const installSkill = useMarkdownHelp(
  ...
  {
    outputShape:
      '{ "success": true, "data": { "directory": "...", "file": "..." } }',
    sideEffects: ["Writes `<skillsDirectory>/superego-cli/SKILL.md`."],
    failureCases: [
      "`--agent` and `--directory` both supplied.",
      "Neither `--agent` nor `--directory` supplied.",
      "`--agent` is not a known agent.",
      "Target directory cannot be written.",
    ],
    relatedCommands: ["superego --help", "superego apps --help"],
  },
);
...
- Create/list/update objects: use proxy commands with JSON input files.
