...
function classifyClosedCycleDay(dayOrdinal, cycle, model, loggedFlow) {
  ...
  const classificationStatus = isLoggedNoFlow ? "logged" : "inferred";
  const withNoFlowRationale = (rationale) =>
...
function classifyForecastDay(dayOrdinal, lastEpisodeStart, model, loggedFlow) {
  ...
  const isLoggedNoFlow = loggedFlow === "None";
  const isLoggedBleeding = isBleedingFlow(loggedFlow);
  const classificationStatus = isLoggedNoFlow ? "logged" : "forecast";
  const withNoFlowRationale = (rationale) =>
    isLoggedNoFlow ? `No flow logged for this day. ${rationale}` : rationale;
  if (isLoggedBleeding) {
    return {
...
export default function App(props) {
      ...
      return React.createElement(
        ...
        React.createElement(
          ...
          flowIndicator
            ? React.createElement(
                Text,
                { element: "span", size: "sm", style: { lineHeight: 1.1 } },
                flowIndicator,
              )
            : null,
          fertilityIndicator
            ? React.createElement(
                Text,
                { element: "span", size: "xs", style: { lineHeight: 1.1 } },
                fertilityIndicator,
              )
            : null,
          isFutureDay &&
  ...
  const renderDayPopover = React.useCallback(
    (day) => {
      ...
      if (isFutureDay) {
        if (prediction.status === "no-data" || prediction.phase === null) {
          return React.createElement(
            ...
            React.createElement(
              Alert,
              { variant: "neutral", title: "Not enough data" },
              React.createElement(
                Text,
        ...
        return React.createElement(
          ...
          React.createElement(
            Text,
            { element: "p", size: "sm", style: { marginTop: "0.5rem" } },
            "Phase:",
            " ",
            React.createElement(
              Text,
              { element: "span", size: "sm", weight: "bold" },
          ...
          prediction.cycleDay !== null && prediction.cycleLength !== null
            ...
            : null,
          prediction.status !== "logged" && prediction.confidenceScore !== null
            ? React.createElement(
                Text,
                { element: "p", size: "sm" },
          ...
          detailsHref
            ? React.createElement(
                ...
                React.createElement(
                  ButtonLink,
                  { size: "sm", href: detailsHref, target: "_top" },
                  "View details",
                ),
              )
            : null,
      ...
      return React.createElement(
        ...
        React.createElement(
          Text,
          { element: "h4", size: "lg", weight: "semibold" },
          "Flow",
        ),
        React.createElement(
          "div",
  ...
  return React.createElement(SimpleMonthCalendar, {
    firstDayOfWeek: "mon",
