import { Alert, ButtonLink, SimpleMonthCalendar, Text, ToggleButton, } from "@superego/app-sandbox/components";
import { useCreateDocument, useCreateNewDocumentVersion, } from "@superego/app-sandbox/hooks";
...
const DEFAULT_PERIOD_FLOW_PATTERN = [
    "Medium",
    "Heavy",
    "Medium",
    "Light",
...
const FLOW_BUTTON_COLOR = "#dc2626";
const PHASE_STYLES = {
    Period: {
...
function localDateToPlainDate(date) {
    const year = date.getFullYear();
    const month = String(date.getMonth() + 1).padStart(2, "0");
    const day = String(date.getDate()).padStart(2, "0");
    return `${year}-${month}-${day}`;
}
function plainDateToOrdinal(plainDate) {
...
function getFertilityIndicator(level) {
    if (level <= 0) {
        return null;
    }
    return "🥚".repeat(level);
...
function isBleedingFlow(flow) {
    return flow === "Light" || flow === "Medium" || flow === "Heavy";
}
function flowToSeverity(flow) {
    if (flow === "Light") {
        return 1;
    }
    if (flow === "Medium") {
...
function extractEpisodes(logsByDate) {
    const explicitNoFlowOrdinals = new Set([...logsByDate.values()]
    ...
    const uniqueOrdinals = bleedingOrdinals.filter((ordinal, index) => index === 0 || ordinal !== bleedingOrdinals[index - 1]);
    const runs = [];
...
function buildCompleteCycles(episodes) {
...
function estimateCycleModelFromLastSix(completeCycles, episodes, loggedFlowsByOrdinal) {
    ...
    const cycleLengthAverage = weightedAverage(cycleLengths, weights) ?? DEFAULT_CYCLE_LENGTH;
    ...
    const sigmaCycle = weightSum > 0 ? Math.sqrt(weightedSquaredDistanceSum / weightSum) : 0;
    const periodDurations = windowCycles.map((cycle) => episodes[cycle.episodeIndex].duration);
    const periodMedian = weightedMedian(periodDurations, weights) ?? DEFAULT_PERIOD_LENGTH;
    const muPeriod = clamp(Math.round(periodMedian), PERIOD_LENGTH_MIN, PERIOD_LENGTH_MAX);
    const flowBuckets = Array.from({ length: PERIOD_LENGTH_MAX }, () => ({
        values: [],
        weights: [],
    }));
    const allFlowValues = [];
    const allFlowWeights = [];
    ...
    const fallbackPeriodFlow = severityToFlow(weightedMedian(allFlowValues, allFlowWeights) ?? 2);
    ...
    const rangeLastSix = cycleLengths.length > 0
...
function buildLoggedFlowsByOrdinal(logsByDate) {
    const loggedFlowsByOrdinal = new Map();
...
function buildPredictionContext(logsByDate) {
    ...
    const model = estimateCycleModelFromLastSix(completeCycles, episodes, loggedFlowsByOrdinal);
    return {
        episodes,
        completeCycles,
        model,
...
function findCompleteCycleContainingDay(dayOrdinal, completeCycles) {
...
function findEpisodeContainingDay(dayOrdinal, episodes) {
...
function fertilityLevelInFertileWindow(dayOrdinal, fertileWindowStart, ovulationDay) {
...
function resolvePeriodFlowForCycleDay(cycleDay, model) {
    const dayIndex = clamp(cycleDay - 1, 0, PERIOD_LENGTH_MAX - 1);
    return model.periodFlowByDay[dayIndex] ?? model.fallbackPeriodFlow;
}
function classifyClosedCycleDay(dayOrdinal, cycle, model, loggedFlow) {
    ...
    const classificationStatus = isLoggedNoFlow
        ? "logged"
        : "inferred";
    const withNoFlowRationale = (rationale) => isLoggedNoFlow ? `No flow logged for this day. ${rationale}` : rationale;
    if (isLoggedBleeding) {
        return {
            status: "logged",
            phase: "Period",
    ...
    const inferredPeriodEnd = Math.min(cycle.start + model.muPeriod - 1, cycle.endExclusive - 1);
    if (!isLoggedNoFlow && dayOrdinal <= inferredPeriodEnd) {
        return {
            ...
            cycleLength: cycle.length,
            cycleStep: 1,
            rationale: "Likely period day from your personalized period duration estimate.",
        };
    }
    ...
    if (dayOrdinal >= fertileWindowStart && dayOrdinal <= ovulationDay) {
        return {
            ...
            phase: "Fertile",
            predictedFlow: null,
            fertilityLevel: fertilityLevelInFertileWindow(dayOrdinal, fertileWindowStart, ovulationDay),
            cycleDay,
            cycleLength: cycle.length,
            cycleStep: 1,
            rationale: withNoFlowRationale("Likely fertile window (6 days ending on estimated ovulation)."),
        };
    }
    if (dayOrdinal < fertileWindowStart) {
        return {
            ...
            cycleLength: cycle.length,
            cycleStep: 1,
            rationale: withNoFlowRationale("Likely follicular phase between period and fertile window."),
        };
    }
    return {
        status: classificationStatus,
        ...
        cycleLength: cycle.length,
        cycleStep: 1,
        rationale: withNoFlowRationale("Likely luteal phase after estimated ovulation."),
    };
}
function classifyForecastDay(dayOrdinal, lastEpisodeStart, model, loggedFlow) {
    const cycleLength = Math.max(1, model.muCycle);
    const cycleStep = Math.max(1, Math.floor((dayOrdinal - lastEpisodeStart) / cycleLength) + 1);
    ...
    if (!isLoggedNoFlow && dayOrdinal <= periodEnd) {
        return {
            ...
            cycleLength,
            cycleStep,
            rationale: "Likely period window projected from your last 6 complete cycles.",
        };
    }
    ...
    if (dayOrdinal >= fertileWindowStart && dayOrdinal <= ovulationDay) {
        return {
            ...
            cycleLength,
            cycleStep,
            rationale: withNoFlowRationale("Likely fertile window (6 days ending on predicted ovulation)."),
        };
    }
    if (dayOrdinal < fertileWindowStart) {
        return {
            ...
            cycleLength,
            cycleStep,
            rationale: withNoFlowRationale("Likely follicular phase before the predicted fertile window."),
        };
    }
    return {
        status: classificationStatus,
        ...
        cycleLength,
        cycleStep,
        rationale: withNoFlowRationale("Likely luteal phase after predicted ovulation."),
    };
}
...
function computeConfidence(status, model, cycleStep) {
    if (status === "logged") {
        return {
            score: 1,
            tier: "High",
...
function predictDay(day, context, log) {
    ...
    let classified = null;
    if (model) {
        const closedCycle = findCompleteCycleContainingDay(dayOrdinal, context.completeCycles);
        if (closedCycle) {
            classified = classifyClosedCycleDay(dayOrdinal, closedCycle, model, loggedFlow);
        }
    }
    if (classified === null &&
        model &&
        ...
        classified = classifyForecastDay(dayOrdinal, context.lastEpisodeStart, model, loggedFlow);
    }
    ...
    const confidence = computeConfidence(classified.status, model, classified.cycleStep);
    const explanation = classified.status === "logged"
        ? null
...
export default function App(props) {
    ...
    const predictionContext = React.useMemo(() => buildPredictionContext(logsByDate), [logsByDate]);
    ...
    const mutationError = createNewDocumentVersion.error?.name ?? createDocument.error?.name ?? null;
    const upsertLog = React.useCallback((day, updater) => {
        if (isMutating) {
            return;
        }
        ...
        if (existingLog) {
            createNewDocumentVersion.mutate(COLLECTION_ID, existingLog.id, existingLog.versionId, next);
        }
        else {
    ...
    const onFlowChange = React.useCallback((day, nextFlow) => {
        upsertLog(day, (log) => ({
            ...log,
            flow: nextFlow,
        }));
    }, [upsertLog]);
    const onSymptomChange = React.useCallback((day, symptom, enabled) => {
        upsertLog(day, (log) => {
    ...
    const renderDayCell = React.useCallback((day) => {
        ...
        const isPredictedDay = prediction.status === "inferred" || prediction.status === "forecast";
        const shouldUseDashedBorder = isPredictedDay &&
            (prediction.phase === "Period" || prediction.phase === "Fertile");
        const flowIndicator = getFlowIndicator(prediction.predictedFlow);
        const fertilityIndicator = prediction.phase === "Fertile"
            ? getFertilityIndicator(prediction.fertilityLevel)
            : null;
        const symptomsPreview = (log?.symptoms ?? []).slice(0, 2);
        return (React.createElement(SimpleMonthCalendar.DayCell, { style: phaseStyle
                ? {
            ...
            React.createElement("div", { style: {
                ...
                flowIndicator ? (React.createElement(Text, { element: "span", size: "sm", style: { lineHeight: 1.1 } }, flowIndicator)) : null,
                fertilityIndicator ? (React.createElement(Text, { element: "span", size: "xs", style: { lineHeight: 1.1 } }, fertilityIndicator)) : null,
                isFutureDay &&
                    !flowIndicator &&
                    !fertilityIndicator &&
                    (prediction.phase === "Period" || prediction.phase === "Fertile") ? (React.createElement(Text, { element: "span", size: "xs", color: "secondary", style: { lineHeight: 1.1, textAlign: "center" } }, phaseDescription(prediction.phase))) : null,
                symptomsPreview.map((symptom, index) => (React.createElement(Text, { key: `${symptom}-${index}`, element: "span", size: "xs", style: {
                        display: "block",
                        lineHeight: 1.1,
                        whiteSpace: "nowrap",
                        overflow: "hidden",
    ...
    const renderDayPopover = React.useCallback((day) => {
        ...
        const detailsHref = logDocument !== null
            ? logDocument.href
            : null;
        const prediction = predictDay(day, predictionContext, log);
        const today = getTodayPlainDate();
        ...
        if (isFutureDay) {
            if (prediction.status === "no-data" || prediction.phase === null) {
                return (React.createElement(SimpleMonthCalendar.DayPopover, null,
                    React.createElement(Alert, { variant: "neutral", title: "Not enough data" },
                        React.createElement(Text, { element: "p", size: "sm", color: "secondary" }, "Record your period to start seeing predictions."))));
            }
            return (React.createElement(SimpleMonthCalendar.DayPopover, null,
                React.createElement(Text, { element: "p", size: "sm", style: { marginTop: "0.5rem" } },
                    "Phase:",
                    " ",
                    React.createElement(Text, { element: "span", size: "sm", weight: "bold" }, phaseLabel(prediction.phase))),
                React.createElement(Text, { element: "p", size: "sm" },
                    "Fertility:",
                    " ",
                    React.createElement(Text, { element: "span", size: "sm", weight: "bold" }, fertilityLabel(prediction.fertilityLevel))),
                prediction.cycleDay !== null && prediction.cycleLength !== null ? (React.createElement(Text, { element: "p", size: "sm" },
                    "Cycle day:",
                    " ",
                    React.createElement(Text, { element: "span", size: "sm", weight: "bold" },
                        prediction.cycleDay,
                        " of ",
                        prediction.cycleLength))) : null,
                prediction.status !== "logged" &&
                    prediction.confidenceScore !== null ? (React.createElement(Text, { element: "p", size: "sm" },
                    "Prediction confidence:",
                    " ",
                    React.createElement(Text, { element: "span", size: "sm", weight: "bold" }, formatConfidenceScore(prediction.confidenceScore)))) : null,
                detailsHref ? (React.createElement("div", { style: {
                        marginTop: "0.75rem",
                        display: "flex",
                        justifyContent: "flex-end",
                    } },
                    React.createElement(ButtonLink, { size: "sm", href: detailsHref, target: "_top" }, "View details"))) : null));
        }
        const currentFlow = log?.flow ?? null;
        const currentSymptoms = log?.symptoms ?? [];
        return (React.createElement(SimpleMonthCalendar.DayPopover, null,
            prediction.status === "no-data" || prediction.phase === null ? (React.createElement(Text, { element: "p", size: "sm", color: "secondary" }, "Not enough data to predict cycle phase for this day.")) : (React.createElement(React.Fragment, null,
                React.createElement(Text, { element: "p", size: "sm" },
                    "Phase:",
                    " ",
                    React.createElement(Text, { element: "span", size: "sm", weight: "bold" }, phaseLabel(prediction.phase))),
                ...
                prediction.status !== "logged" &&
                    ...
                    " ",
                    React.createElement(Text, { element: "span", size: "sm", weight: "bold" }, formatConfidenceScore(prediction.confidenceScore)))) : null)),
            React.createElement(Text, { element: "h4", size: "lg", weight: "semibold" }, "Flow"),
            React.createElement("div", { style: {
                    marginTop: "0.5rem",
                    display: "grid",
                    gridTemplateColumns: "repeat(3, minmax(0, 1fr))",
                    gap: "0.5rem",
                } }, FLOW_OPTIONS.map((option) => (React.createElement(ToggleButton, { key: option.label, size: "lg", fullWidth: true, color: FLOW_BUTTON_COLOR, value: currentFlow === option.value, onChange: (nextValue) => {
                    if (isMutating) {
                        return;
                    }
                    onFlowChange(day, nextValue ? option.value : null);
                } },
                React.createElement(Text, { element: "span", size: "sm", weight: "medium", style: { color: "inherit", padding: "0.25rem 0" } }, option.label))))),
            React.createElement(Text, { element: "h4", size: "lg", weight: "semibold" }, "Symptoms"),
            React.createElement("div", { style: {
                    marginTop: "0.5rem",
                    display: "grid",
                    gridTemplateColumns: "repeat(2, minmax(0, 1fr))",
                    gap: "0.5rem",
                } }, COMMON_SYMPTOMS.map((symptom) => (React.createElement(ToggleButton, { key: symptom, size: "lg", fullWidth: true, color: "#2563eb", value: currentSymptoms.includes(symptom), onChange: (nextValue) => {
                    if (isMutating) {
                        return;
                    }
                    onSymptomChange(day, symptom, nextValue);
                } },
                React.createElement(Text, { element: "span", size: "sm", weight: "medium", style: { color: "inherit", padding: "0.25rem 0" } }, capitalize(symptom)))))),
            mutationError ? (React.createElement(Alert, { variant: "error", title: "Could not save changes", style: { marginTop: "1rem" } },
                React.createElement(Text, { element: "p", size: "sm", color: "secondary" },
                    "Error: ",
                    mutationError))) : null,
            detailsHref ? (React.createElement("div", { style: {
                    marginTop: "1rem",
